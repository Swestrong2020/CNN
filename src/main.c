...
//         SW_AddNetworkLayer(&network, 1, SW_ACTIVATION_FUNCTION_RELU);
...
//         SW_SaveNetwork(&network, "testnetwork");

//         SW_Network networkLoaded;
//         SW_InitNetwork(&networkLoaded);

//         SW_LoadNetwork(&networkLoaded, "testnetwork");



//         SW_SetNetworkInput(&network, &testInut);
//         SW_SetNetworkInput(&networkLoaded, &testInut);

//         SW_ExucuteNetwork(&network);
//         SW_ExucuteNetwork(&networkLoaded);

//         printf("output original: %e, output loaded: %e\n", network.layers[network.layerAmount-1].neurons[0].output, networkLoaded.layers[networkLoaded.layerAmount-1].neurons[0].output);
    
//         SW_UnloadNetwork(&network);
//         SW_UnloadNetwork(&networkLoaded);
//     }
